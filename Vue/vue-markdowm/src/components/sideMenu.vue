		...
		<ul class="files">
			<template v-for="i in articleList.length">
				<li :class="{'current': articleList[i-1].current}">
					...
					<button class="delet-btn" v-if="articleList.length > 1"  @click="deleteThis(i)"></button>
				</li>
			</template>
		</ul>
		<ul class="options">
...
<script>
	export default {
		...
		computed: {
			articleList () {
				return this.$store.getters.artcleList
			},
			titleHtml () {
				return this.$store.getters.articleRaw.split('\n')[0] + '.html'
			},
			titleMd () {
				return this.$store.getter.articleRaw.split('\n')[0] + '.md'
			}
		},
		methods: {
			selectThis (i) {
			...
			newArticle () {
				...
				let fliesBox = document.querySelector('.files')
				setTimeout(() =>{
					fliesBox.scrollTop = fliesBox.scrollHeight + 180
				}, 100)
			},
			saveToCache () {
