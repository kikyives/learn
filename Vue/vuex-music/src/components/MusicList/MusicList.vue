<template lang="html">
	<transition name="showRouter">
		<div class="music-list">
			<div class="music-item" v-for="(item, index) of musicData">
				<img class="music-img" v-bind:src="item.musicImgSrc || defaultImg" />
				<span class="music-name" @click="toggleMusic(index)">{{ (index+1) + '&nbsp;' + item.name}}</span>
...
<script>
	export default {
		...
		methods: {
			toggleMusic(index) {
				if(this.$store.audio.index === index) {
					this.DOM.audio.paused ? this.DOM.audio.play() : this.DOM.audion.pause();
